...
(Corollary~4.5 of Paper~0) demands that each resolution be atomic.
...
\begin{proposition}[Allocation bias induces IML drift]
\label{prop:bias-drift}
If $\Delta_{u_j}(t)$ is monotonically increasing, then the
admission-time behavioral contract $\mathcal{A}_0$ of agent
$a_i \in \Agents_j$ is violated in the following sense: the
action-frequency distribution of $a_i$ diverges from $P_{E_0}$ even
if all individual actions are admissible.  Specifically, an
over-served agent issues actions at a rate above its
admission-time expectation, increasing the IML deviation
$\widehat{D}(\tau_i, \mathcal{A}_0)$ of \citet{fernandez2026c},
Theorem~3.
\end{proposition}

\begin{proof}[Proof sketch]
The admission-time snapshot $\mathcal{A}_0^{\mathrm{emp}} = (P_{E_0},
\mu_{\mathrm{depth}}, \sigma_{\mathrm{depth}})$ of
\citet{fernandez2026c} is constructed during a burn-in period under
fair allocation (equal access).  If $a_i$ subsequently receives
$\Share{i} \gg 1/N$, its empirical action-frequency distribution
$\hat{P}_{\tau_i}$ will shift toward boundary and risky tools
(by the same mechanism as the tool-drift scenario of
\citet{fernandez2026c}, Experiment~1), increasing
$\mathrm{JS}(\hat{P}_{\tau_i} \| P_{E_0}) > 0$ and thus $\widehat{D}_t
\nearrow$.  The IML enforcement signal $g(\tau_i) = 0$ throughout,
since no individual action violates a constraint.
\end{proof}

Proposition~\ref{prop:bias-drift} establishes a bridge between the
...
decision is incorrect.  Conversely, a well-calibrated allocation
mechanism prevents the IML from being flooded with allocation-induced
false positives (agents appearing to drift when they are merely
over-served).
...
Several important questions are left for future work.  First, we assume
...
$K_U$ when actors join or leave).  Second, the actor mapping $\owner$
is assumed known to the mechanism in M3 and M4(actor-wt); in practice,
$\owner$ may be unobservable or strategically misrepresented.  Third,
we analyze worst-case fairness; a stochastic model of request arrivals
would permit average-case analysis and tighter detection-delay bounds
for the allocation bias signal of \S\ref{sec:alloc-bias}.  Finally, the
interaction between the allocation layer and ACP's Boundary Activation
Rate (BAR-Monitor)~\citep{fernandez2026b} deserves formal treatment:
M3 may reduce BAR by concentrating enforcement events on under-served
agents, which BAR could misinterpret as deviation collapse.

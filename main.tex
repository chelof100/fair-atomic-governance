...
\newcommand{\Agents}{\mathcal{Ag}}         % set of agents (series: distinct from \mathcal{A}_0 in Paper 2)
...
This paper is Paper~3 of a 6-paper Agent Governance Series:
atomic decision boundaries (P0,~\citet{fernandez2026a}),
stateful enforcement---ACP (P1,~\citet{fernandez2026b}),
behavioral drift detection---IML (P2,~\citet{fernandez2026c}),
composition irreducibility (P4,~\citet{fernandez2026comp}),
and runtime execution validity---RAM (P5,~\citet{fernandez2026ram}).
\end{abstract}
...
This paper is Paper~3 in a series of six.  Paper~0~\citep{fernandez2026a}
...
behavioral drift above the boundary.  The present paper addresses the
allocation layer: given a correctly functioning atomic boundary and a
behavioral invariant monitor, how should the governance infrastructure
be shared among competing agents so that no actor can disproportionately
capture the system's execution capacity?
...
\label{rem:notation-agents}
This paper uses $\Agents$ for the \emph{agent population set}.
In Paper~2~\citep{fernandez2026c}, the symbol $\mathcal{A}_0$ denotes
the \emph{admissible behavior space}---a completely different object
(a subset of trace sequences, not a set of agents).
The two objects never appear in the same formula; the subscript~$0$
unambiguously identifies Paper~2's object wherever both papers are read
together.  For cross-paper readability, this paper writes $\Agents$
(rendered $\mathcal{Ag}$) to make the distinction typographically
explicit.
\end{remark}
...
\caption{Failure modes and the fairness properties they violate.
  $\checkmark$ = the failure mode does \emph{not} violate the property
  (consistent); $\times$ = direct violation.
  Note: Strategy-Proofness (Definition~\ref{def:strategy-proof})
  concerns identity fragmentation specifically.
  Temporal domination and resource contention exploit timing and
  volume, not agent splitting---hence they are consistent with
  strategy-proofness as defined, but may still degrade overall
  fairness through the other dimensions.}
...
\bottomrule
\multicolumn{4}{l}{\footnotesize $^*$ Strategy-Proofness is defined against
  identity fragmentation (Def.~\ref{def:strategy-proof}); \checkmark}\\
\multicolumn{4}{l}{\footnotesize \quad here means the mode does not exploit
  agent splitting, not that the mechanism is immune to the mode.}
\end{tabular}
\end{table}
...
$\Share{u_j} = m_j/M$.  Since there are at most as many actors as
agents ($M \leq N$), we have $1/M \geq 1/N$, and multiplying both
sides by $m_j > 0$ yields $m_j/M \geq m_j/N$;  thus
...
as given by Theorem~3.6 of \citet{fernandez2026c}, satisfies:
\[
...
Papers~0--3 define four distinct, non-redundant layers of governance.
Figure~\ref{fig:fourlayer} depicts their relationship.  Each layer
addresses a failure mode invisible to the layer below it:
enforcement-time state changes (Paper~0), per-action admissibility
(Paper~1), within-compliance behavioral drift (Paper~2), and
cross-agent allocation skew (Paper~3).
Paper~4~\citep{fernandez2026comp} proves this four-layer architecture
is irreducible.  Paper~5~\citep{fernandez2026ram} closes the series by
providing the runtime operational mechanism for execution validity under
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=3pt, minimum width=8.4cm, text width=7.8cm,
...
that is necessary but not sufficient alone, and the series is complete
exactly when all six components are present.
...
We validate the main results with three controlled experiments using a
purpose-built harness that implements M1, M2, and M3 directly from their
formal definitions (Section~\ref{sec:mechanisms}).  All experiments use
an adversarial Sybil scenario: two honest actors each controlling two
agents, and one adversarial actor controlling a variable number of Sybil
agents.  Results are averaged over 10 independent seeds; full code and
CSVs are included in the supplementary material.

\subsection{Exp A: Actor Share Distribution (M1 vs.\ M2 vs.\ M3)}
\label{sec:exp-a}

We run 500 steps with 8 Sybil agents (total agents: 4 honest + 8 Sybil)
and compare the final actor shares under each mechanism.
Figure~\ref{fig:share-dist} shows the results.

\begin{figure}[ht]
\centering
\includegraphics[width=0.82\columnwidth]{experiments/figures/fig1_share_distribution}
\caption{Actor share distribution.  M1 and M2 are identity-oblivious:
  with 8 Sybil agents the adversary captures $0.667$ of all allocations
  ($2\times$ the fair share $1/3$).  M3 bounds the adversary to exactly
  $0.333 = 1/M$ in \emph{every} seed.}
\label{fig:share-dist}
\end{figure}

\noindent
M1 and M2 both yield adversary share $= 0.667$ (zero variance across all
10 seeds), confirming Theorem~\ref{thm:sybil}: per-agent bounded
enforcement scales linearly with agent count when the allocator is
identity-oblivious.  M3 yields adversary share $= 0.333 \pm 0.000$,
matching the theoretical bound $K_U/K_\mathrm{cap} = 1/M$ of
Proposition~\ref{prop:m3}.

\subsection{Exp B: Sybil Amplification Curve}
\label{sec:exp-b}

We vary the number of Sybil agents from 1 to 16 and measure the adversary's
share under each mechanism (Figure~\ref{fig:sybil-amp}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.82\columnwidth]{experiments/figures/fig2_sybil_amplification}
\caption{Sybil amplification (Theorem~\ref{thm:sybil}).  Under M1 and M2
  the adversary share grows from $0.20$ (1 Sybil agent) to $0.80$
  (16 agents), tracking $n_s/(n_s+4)$.  Under M3 the share stays flat
  at $0.333$ regardless of $n_s$.}
\label{fig:sybil-amp}
\end{figure}

\noindent
Under M1 and M2 the adversary share follows the linear amplification
$n_s/(n_s + n_h)$ predicted by Theorem~\ref{thm:sybil}
(with $n_h = 4$ honest agents, the formula gives
$1/(1+4)=0.20$, $8/(8+4)=0.667$, $16/(16+4)=0.80$,
matching the observed values exactly).  Under M3, the share is flat at
$0.333$ for all $n_s \in \{1,\ldots,16\}$, with maximum deviation
$0.0002$ from the ideal $1/3$.

\subsection{Exp C: Strategy-Proofness}
\label{sec:exp-c}

We compare the marginal gain from registering additional Sybil agents under
M1 (not strategy-proof) and M3 (strategy-proof), to validate
Proposition~\ref{prop:m3} empirically (Figure~\ref{fig:sp}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\columnwidth]{experiments/figures/fig3_strategy_proofness}
\caption{Strategy-proofness.  \emph{Left}: adversary share vs.\ Sybil
  count.  \emph{Right}: marginal gain above fair share ($1/3$).  Under M3
  the gain is $\approx 0$ for all Sybil counts (max $0.0002$); under M1
  the gain grows linearly to $+0.46$ at 16 agents.}
\label{fig:sp}
\end{figure}

\noindent
Under M3, the gain above fair share is zero ($\le 0.0002$) regardless of
how many Sybil agents the adversary registers, confirming
strategy-proofness: no actor can increase its allocation by fragmenting
its identity.  Under M1, the gain grows from $-0.133$ (1 agent, below
fair) to $+0.459$ (16 agents), establishing that M1 is not
strategy-proof.  Together, Experiments A--C provide empirical support for
Theorems~\ref{thm:sybil} and~\ref{thm:necessity} and
Proposition~\ref{prop:m3}.
...
\subsection{Position within the Agent Governance Series}
\label{sec:related-quartet}

The six papers in this series form a logically closed architecture;
Table~\ref{tab:quartet} summarizes each paper's question, result, and
the failure mode it addresses.

\begin{table}[h]
\centering
\caption{The Agent Governance Series: question, result, and failure mode addressed.}
\label{tab:quartet}
\small
...
This paper completes the fairness layer of the Agent Governance Series.
Paper~0~\citep{fernandez2026a} establishes that admissibility can only
be guaranteed by atomic enforcement: the decision boundary is a
structural necessity, not an engineering choice.
Paper~1~\citep{fernandez2026b} instantiates the boundary as ACP and
demonstrates its correctness properties across thousands of states.
Paper~2~\citep{fernandez2026c} shows that enforcement-correct behavior
can still drift from admission-time contracts, and introduces IML to
detect this drift.  The present paper shows that a drift-monitored,
atomically correct system can still allocate its governance capacity
unfairly---and provides the formal tools to characterize, prevent, and
detect this failure.  Paper~4~\citep{fernandez2026comp} proves the
irreducibility of the resulting four-layer architecture.
Paper~5~\citep{fernandez2026ram} closes the series with the
Reconstructive Authority Model: the runtime mechanism that determines
whether execution authority can be constructed from the current real
state, operationalizing the observability limits proved by Paper~2.
